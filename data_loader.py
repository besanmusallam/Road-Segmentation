...
def load_data(image_dir, mask_dir, img_size=(400, 400)):
    ...
    for f in mask_files:
        ...
        if mask is None:
            print(f"Warning: Failed to load mask {f}")
        else:
            masks.append(mask)

    # Filter out None values from the images and masks
    images = [img[..., np.newaxis] for img in images if img is not None]
    masks = [mask[..., np.newaxis] for mask in masks if mask is not None]
